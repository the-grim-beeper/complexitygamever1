    ...
    with col2:
        ...
        if action == "Invest in Technology":
            ...
            for inst, data in game.institutions.items():
                ...
                inst_options.append(f"{data['icon']} {inst} (Cost: {cost})")
            
            selected_inst_display = st.selectbox("Select institution:", inst_options)
            
            if selected_inst_display:
                # Extract inst name from display string
                inst_name = selected_inst_display.split(" (")[0]
                inst_name = inst_name[2:]  # Remove icon
                st.session_state.selected_inst = inst_name
                
                # Show institution details
                data = game.institutions[inst_name]
                current_level = data['level']
                ...
                if st.button("Upgrade", disabled=not can_upgrade):
                    success, message = game.invest_in_institution(inst_name)
                    st.session_state.message = message
